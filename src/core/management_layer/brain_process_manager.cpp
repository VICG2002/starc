...
struct BrainProcessManager::Implementation {
    ...
    void writeHermesAgentsMd()
    {
        const QString md = QStringLiteral(
            ...
            "## Actualizar/redeploy Aula 122 (semi-autonomo, GATE DE CONFIRMACION)\n\n"
            "Tienes tools para administrar updates del propio software (Odiseo + tu cerebro):\n"
            "`revisar_actualizacion` (status), `respaldar_aula122` (backup), `redesplegar_aula122`.\n\n"
            "- revisar_actualizacion y respaldar_aula122 son SEGUROS: usalos cuando quieras.\n"
            "- redesplegar_aula122 es DESTRUCTIVO (recompila + RELANZA la app, te reinicia a ti).\n"
            "  FLUJO: 1) revisar_actualizacion y resume; 2) respaldar_aula122; 3) llama\n"
            "  redesplegar_aula122(confirmar=true) SIN token -> NO aplica, deja un token de un\n"
            "  solo uso en un archivo; 4) PIDELE a Victor que abra ese archivo y te dicte el\n"
            "  token (NUNCA lo leas tu); 5) redesplegar_aula122(confirmar=true, token=<lo dicho>).\n"
            "- NUNCA `git push` ni re-vendorizado upstream (manual de Victor).\n"
            "- Si NO hay humano en la conversacion (cron/autonomo), NO redespliegues.\n\n"
