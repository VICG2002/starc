...
struct BrainProcessManager::Implementation {
    ...
    void writeHermesAgentsMd()
    {
        const QString md = QStringLiteral(
            ...
            "## Actualizar/redeploy Aula 122 (semi-autonomo, CON CONFIRMACION)\n\n"
            "Hay una utilidad GUARDADA para recompilar y redesplegar cambios del propio\n"
            "software (Odiseo + tu cerebro). Invocala por terminal:\n"
            "`bash ~/Developer/starc-fork/ai/aula122-update.sh <cmd>`.\n\n"
            "- `status` y `backup` son SEGUROS (solo lectura / respaldo): correlos cuando quieras.\n"
            "- `rebuild` es DESTRUCTIVO (recompila y RELANZA la app, te reinicia a ti incluido).\n"
            "  FLUJO OBLIGATORIO: 1) corre `status` y resume que cambiaria; 2) corre `backup`;\n"
            "  3) PIDE CONFIRMACION EXPLICITA a Victor en el chat; 4) SOLO si confirma, corre\n"
            "  `rebuild --confirm`. Sin `--confirm` el script hace dry-run (no toca nada).\n"
            "- NUNCA hagas `git push` ni re-vendorices Hermes/odysseus desde upstream: eso es\n"
            "  manual de Victor. La utilidad tampoco los hace.\n"
            "- Si NO hay un humano en la conversacion (cron/autonomo), NO ejecutes `rebuild`.\n\n"
