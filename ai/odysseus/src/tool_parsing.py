        ...
        for m in _XML_TOOL_CALL_RE.finditer(text):
            ...
            for inv in _XML_INVOKE_RE.finditer(inner):
                ...
                if block:
                    blocks.append(block)
                    matched = True
            # Qwen/Hermes JSON style: <tool_call>{"name": "...", "arguments": {...}}</tool_call>
            # (no <invoke> inside — the body is a JSON object). Common with
            # Ollama-served Qwen/Hermes models that emit their native tool
            # format as text instead of structured tool_calls.
            if not matched:
                start, end = inner.find("{"), inner.rfind("}")
                if start != -1 and end > start:
                    try:
                        obj = json.loads(inner[start:end + 1])
                    except Exception:
                        obj = None
                    if isinstance(obj, dict) and obj.get("name"):
                        from src.tool_schemas import function_call_to_tool_block
                        _args = obj.get("arguments", obj.get("parameters", {}))
                        _args_str = _args if isinstance(_args, str) else json.dumps(_args or {})
                        block = function_call_to_tool_block(str(obj["name"]), _args_str)
                        if block:
                            blocks.append(block)
